...
class Publisher(Node):
    ...
    def __init__(self):
        ...
        self.publisher = self.create_publisher(Float32MultiArray, 'tag_poses', 10)
        timer_period = 2
        ...
        while cap.isOpened():
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    vector_magnitude = float (sqrt (tvecs[0][0]*tvecs[0][0]+tvecs[0][1]*tvecs[0][1] + tvecs[0][2]*tvecs[0][2])) / 100.0
                    msg.data = [float (ids[0]), (float (tvecs[0][0]))/vector_magnitude + tag_x_offset, float (tvecs[0][1])/vector_magnitude+tag_y_offset, float (tvecs[0][2])/10.0+tag_z_offset, float (rvecs[0][2])] #just publishing the first element in tvecs
                    print('Publishing data: ', msg.data)
